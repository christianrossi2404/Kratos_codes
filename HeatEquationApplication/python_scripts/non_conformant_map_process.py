...
class NonConformatMapProcess(Process):
    ...
    def __init__(self,model_part,params):
        ...
        default_parameters = Parameters("""
        {
            ...
            "tol": 1.0e-3,
            "sign": "Positive",
            "f2s_origin_variables": ["TEMPERATURE"],
            "f2s_destination_variables": ["TEMPERATURE"]
        }
        """)
    ...
    def ExecuteInitializeSolutionStep(self):
        ...
        for i in range(self.params["s2f_origin_variables"].size()):
              ...
              if value_scalar:
                self.mapper.SolidToFluid_ScalarMap(variable_origin, variable_destination, sign_pos)
              else: # It is a vector
                self.mapper.SolidToFluid_VectorMap(variable_origin, variable_destination, sign_pos)
        ...
        for i in range(self.params["f2s_destination_variables"].size()):
              ...
              if value_scalar:
                self.mapper.FluidToSolid_ScalarMap(variable_origin, variable_destination)
              else: # It is a vector
                self.mapper.FluidToSolid_VectorMap(variable_origin, variable_destination)
