...
def ConnectivityMapper(fluid_nodes, solid_nodes):
    ...
    connectivity_list=[]
    id_list=[]
    tol=1.5e-3 #Geometric tolerance.
    i=0
    ...
    for node in fluid_nodes:
        ...
        for node_solid in solid_nodes:
            ...
            z_solid=node_solid.Z
            if x_solid<suptolx and x_solid>inftolx and y_solid<suptoly and y_solid>inftoly and z_solid<suptolz and z_solid>inftolz:
...
ProjectParameters = Parameters("""
{
    ...
    "fluid_settings" :
    {
        ...
        "fluid_solver_settings": {
            ...
            "skin_parts": ["AutomaticInlet3D_INLET","Outlet3D_OUTLET","NoSlip3D_WALLS","NoSlip3D_FLUID_INTERFACE"],
            "no_skin_parts":[""],
            "time_stepping"                :
            {
...
fluid_interface_nodes = thermal_main_model_part.GetSubModelPart("NoSlip3D_FLUID_INTERFACE").Nodes
solid_interface_nodes  = solid_main_model_part.GetSubModelPart("NoSlip3D_SOLID_INTERFACE").Nodes
# print(fluid_nodes)

# fluid_nodes=list(fluid_interface_nodes)
# solid_nodes=list(solid_interface_nodes)
# print(len(fluid_nodes))
# print(len(solid_nodes))
# # connectivity id_nodes matrix
# id_list =ConnectivityMapper(fluid_nodes, solid_nodes)
# # print(id_list)
# print("number of matching nodes :", len(id_list))
# err
interface_fluid_nodes = VariableUtils().SetFlag(INTERFACE, True, thermal_main_model_part.GetSubModelPart("NoSlip3D_FLUID_INTERFACE").Nodes)
solid_interface_nodes = VariableUtils().SetFlag(INTERFACE, True, solid_main_model_part.GetSubModelPart("NoSlip3D_SOLID_INTERFACE").Nodes)

interface_fluid_condition = VariableUtils().SetFlag(INTERFACE, True, thermal_main_model_part.GetSubModelPart("NoSlip3D_FLUID_INTERFACE").Conditions)
interface_solid_condition = VariableUtils().SetFlag(INTERFACE, True, solid_main_model_part.GetSubModelPart("NoSlip3D_SOLID_INTERFACE").Conditions)

interface_fluid_elements = VariableUtils().SetFlag(INTERFACE, True, thermal_main_model_part.GetSubModelPart("NoSlip3D_FLUID_INTERFACE").Elements)
interface_solid_elements = VariableUtils().SetFlag(INTERFACE, True, solid_main_model_part.GetSubModelPart("NoSlip3D_SOLID_INTERFACE").Elements)

thermal_side = thermal_main_model_part.GetSubModelPart("NoSlip3D_FLUID_INTERFACE")
solid_side = solid_main_model_part.GetSubModelPart("NoSlip3D_SOLID_INTERFACE")

# Set fluid and structure interfaces
for node in thermal_main_model_part.GetSubModelPart("NoSlip3D_FLUID_INTERFACE").Nodes:
    node.Set(INTERFACE, True)
for node in solid_main_model_part.GetSubModelPart("NoSlip3D_SOLID_INTERFACE").Nodes:
    node.Set(INTERFACE, True)
...
for node in thermal_main_model_part.GetSubModelPart("AutomaticInlet3D_INLET").Nodes:
...
# outlet   V = 0
for node in fluid_main_model_part.GetSubModelPart("Outlet3D_OUTLET").Nodes:
...
# no-slip condition
for node in fluid_main_model_part.GetSubModelPart("NoSlip3D_WALLS").Nodes:
    node.SetSolutionStepValue(VELOCITY, fluid_wall_velocity)
    node.Fix(VELOCITY_X)
    node.Fix(VELOCITY_Y)
    node.Fix(VELOCITY_Z)
# no-slip condition
for node in fluid_main_model_part.GetSubModelPart("NoSlip3D_FLUID_INTERFACE").Nodes:
    node.SetSolutionStepValue(VELOCITY, fluid_wall_velocity)
    node.Fix(VELOCITY_X)
    node.Fix(VELOCITY_Y)
    node.Fix(VELOCITY_Z)
...
for node in solid_main_model_part.Nodes:
    node.SetSolutionStepValue(TEMPERATURE,initial_solid_temperature)
        ...
        #       if (step % 4 == 0):
        #           alpha = 1/T    # thermal expansion coefficient
        #           g = g * alpha * (T - TEMPERATURE_) #Boussinesq
...
only_fluid = 5/delta_time+1 # solve only the fluid flow during the first 5 seconds
all_model = only_fluid + 2
const = np.arcsin(1)/(only_fluid-1)*2/math.pi
...
for step in range(1,total_steps):
    ...
    if (step<only_fluid):
        ...
        for node in fluid_main_model_part.GetSubModelPart("AutomaticInlet3D_INLET").Nodes:
            Inlet_velocity = Vector(3)
         #U (0, y, z, t) = 16U m yz(H − y)(H − z)/H 4 , V = W = 0
            Inlet_velocity[0] = 16*v_avarage*node.Y*node.Z*(0.2 - node.Y)*(0.2 - node.Z)*math.sin(const*math.pi/2*time)/(0.2**4)
            #Inlet_velocity[0] = 16*v_avarage*node.Y*node.Z*(0.2 - node.Y)*(0.2 - node.Z)/(0.2**4)
        ...
        # solid_temp_list = []
        # for node in solid_main_model_part.GetSubModelPart("NoSlip3D_SOLID_INTERFACE").Nodes:
        #     solid_temp_list.append(solid_main_model_part.GetSubModelPart("NoSlip3D_SOLID_INTERFACE").Nodes[id_list[i][1]].GetSolutionStepValue(TEMPERATURE))
        #     i += 1
        ...
        # fluid_therma_list = []
        for node in thermal_main_model_part.GetSubModelPart("NoSlip3D_FLUID_INTERFACE").Nodes:
        #     thermal_main_model_part.GetSubModelPart("NoSlip3D_FLUID_INTERFACE").Nodes[id_list[k][0]].SetSolutionStepValue(TEMPERATURE, solid_temp_list[k]) # solid_temp_list[k]
            node.Fix(TEMPERATURE)
        #     k += 1
        ...
        # i=0
        # for node in thermal_main_model_part.GetSubModelPart("NoSlip3D_FLUID_INTERFACE").Nodes:
        #     fluid_flux_list.append(thermal_main_model_part.GetSubModelPart("NoSlip3D_FLUID_INTERFACE").Nodes[id_list[i][0]].GetSolutionStepValue(FACE_HEAT_FLUX))
        #     i += 1
        ...
        # k = 0
        for node in solid_main_model_part.GetSubModelPart("NoSlip3D_SOLID_INTERFACE").Nodes:
        #     solid_main_model_part.GetSubModelPart("NoSlip3D_SOLID_INTERFACE").Nodes[id_list[k][1]].SetSolutionStepValue(FACE_HEAT_FLUX,fluid_flux_list[k])
