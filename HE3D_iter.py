...
fluid_nodes = thermal_main_model_part.GetSubModelPart("NoSlip3D_FLUID_INTERFACE").Nodes
solid_nodes  = solid_main_model_part.GetSubModelPart("NoSlip3D_SOLID_INTERFACE").Nodes
...
for node in fluid_main_model_part.GetSubModelPart("Outlet3D_OUTLET").Nodes:
    ...
    node.Fix(VELOCITY_Y)
    node.Fix(PRESSURE)
    #loop neu nodi e se la velocita é negativa  -> forzarla a zero
    ...
    if (step > 2):
        if (step<=only_fluid):
            for node in fluid_main_model_part.GetSubModelPart("AutomaticInlet3D_INLET").Nodes:
                ...
                Inlet_velocity[0] = 16*v_avarage*node.Y*node.Z*(H - node.Y)*(H - node.Z)*math.sin(math.pi/2*time)/(H**4)
            ...
            for node in fluid_main_model_part.GetSubModelPart("AutomaticInlet3D_INLET").Nodes:
                ...
                Inlet_velocity[0] = Inlet_velocity[0] = 16*v_avarage*node.Y*node.Z*(H - node.Y)*(H - node.Z)/(H**4)
                Inlet_velocity[1] = 0.0
                Inlet_velocity[2] = 0.0
                node.SetSolutionStepValue(VELOCITY, Inlet_velocity)
        ...
        for k  in range(1,coupling_max_iter):
            ...
            for node in range(1,len(id_list)):#solid_main_model_part.GetSubModelPart("NoSlip3D_SOLID_INTERFACE").Nodes:
                solid_Gamma.append(solid_main_model_part.GetSubModelPart("NoSlip3D_SOLID_INTERFACE").Nodes[id_list[i][1]].GetSolutionStepValue(TEMPERATURE))
            ...
            for node in fluid_main_model_part.GetSubModelPart("Outlet3D_OUTLET").Nodes:
                pressure = node.Y * node.GetSolutionStepValue(DENSITY) * g0
            ...
            i=0
            for node in range(1,len(id_list)):#thermal_main_model_part.GetSubModelPart("NoSlip3D_FLUID_INTERFACE").Nodes:
                fluid_Gamma.append(thermal_main_model_part.GetSubModelPart("NoSlip3D_FLUID_INTERFACE").Nodes[id_list[i][0]].GetSolutionStepValue(TEMPERATURE))
                i += 1
