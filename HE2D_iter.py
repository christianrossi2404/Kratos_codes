...
for step in range(1,total_steps):
    ...
    if (step <= only_fluid):
        ...
        for node in fluid_main_model_part.GetSubModelPart("AutomaticInlet2D_INLET").Nodes:
            Inlet_velocity[0] = 4*v_avarage*abs(node.Y)*v_avarage*time/only_fluid*(H-abs(node.Y))/(H*H)#4*v_avarage*abs(node.Y)*math.sin(math.pi/2*const*time)*(H-abs(node.Y))/(H*H)
            node.SetSolutionStepValue(VELOCITY, Inlet_velocity)
            node.Fix(VELOCITY_X)
            node.Fix(VELOCITY_Y)
            node.Fix(VELOCITY_Z)
        ...
        for node in fluid_main_model_part.GetSubModelPart("AutomaticInlet2D_INLET").Nodes:
            Inlet_velocity[0] = 4*v_avarage*abs(node.Y)*(H-abs(node.Y))/(H*H)
            node.SetSolutionStepValue(VELOCITY, Inlet_velocity)
            node.Fix(VELOCITY_X)
            node.Fix(VELOCITY_Y)
            node.Fix(VELOCITY_Z)
    ...
    if (step > 2):
        ...
        for k  in range(1,coupling_max_iter):
            ...
            for node in solid_main_model_part.GetSubModelPart("NoSlip2D_SOLID_INTERFACE").Nodes:
                solid_Gamma.append(solid_main_model_part.GetSubModelPart("NoSlip2D_SOLID_INTERFACE").Nodes[id_list[i][1]].GetSolutionStepValue(TEMPERATURE))
                i += 1
            ...
            i=0
            for node in thermal_main_model_part.GetSubModelPart("NoSlip2D_FLUID_INTERFACE").Nodes:
                fluid_Gamma.append(thermal_main_model_part.GetSubModelPart("NoSlip2D_FLUID_INTERFACE").Nodes[id_list[i][0]].GetSolutionStepValue(TEMPERATURE))
                i += 1
